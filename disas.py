...
"CPFSEQ	f, a	WREG と f を比較し、等しければスキップ		1	1 – 4	0110 001a ffff ffff	なし	1, 2",
"CPFSGT	f, a	WREG と f を比較し、大きければスキップ		1	1 – 4	0110 010a ffff ffff	なし	1, 2",
"CPFSLT	f, a	WREG と f を比較し、小さければスキップ		1	1 – 4	0110 000a ffff ffff	なし	1, 2",
"DECFSZ	f, d, a	f をデクリメントし、0 ならスキップ		1	1 – 4	0010 11da ffff ffff	なし	1, 2",
"DCFSNZ	f, d, a	f をデクリメントし、0 でなければスキップ		1	1 – 4	0100 11da ffff ffff	なし	1, 2",
"INCFSZ	f, d, a	f をインクリメントし、0 ならスキップ		1	1 – 4	0011 11da ffff ffff	なし	1, 2",
"INFSNZ	f, d, a	f をインクリメントし、0 でなければスキップ		1	1 – 4	0100 10da ffff ffff	なし	1, 2",
"TSTFSZ	f, a	f をテストし、0 ならスキップ		1	1 – 4	0110 011a ffff ffff	なし	1, 2",
"BCF	f, b, a	f のビットをクリア		1	1	1001 bbba ffff ffff	なし	1",
"BSF	f, b, a	f のビットをセット		1	1	1000 bbba ffff ffff	なし	1",
"BTG	f, b, a	f のビットをトグル		1	1	0111 bbba ffff ffff	なし	1",
"BTFSC	f, b, a	f のビットをテストし、クリアならスキップ		1	1 – 4	1011 bbba ffff ffff	なし	1, 2",
...
def compare_inst18f26q84(a, b) -> int:
    aa, bb = toint2(a, b)
    if aa == bb:
        val = custom_strcmp(a[4], b[4])
    return aa - bb
...
def init_opcode18f26q84():
    ...
    if not _init_opcode:
        ...
        for inst in _instructions18f26q84:
            cols = inst.split("\t")
            temp.append(cols)
        instructions = [[row[0], row[1], row[2], row[3], row[5].replace(" ", "")] for row in temp]
